    ...
    <div className="space-y-8 px-4 py-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="space-x-2">
          {priority && (
            <span className="rounded-md bg-red-400 p-2 tracking-wide text-red-50">
              Priority
            </span>
          )}
          <span className="rounded-md bg-green-400 p-2 tracking-wide text-green-50">
            {status} order
          </span>
        </div>
      </div>
      ...
      <ul className="divide-y divide-stone-300 border-b border-t">
        {cart.map((item) => (
          <OrderItem
            ...
            ingredients={
              fetcher?.data?.find((el) => el.pizzaId === item.id).ingredients
            }
          />
        ))}
      </ul>
